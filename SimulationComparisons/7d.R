...
p3 <- simulation_data_long_mt %>% 
  filter(type %in% c('New conjunctive cell input models', 'Experimental data')) %>%
  ...
  theme(strip.background = element_blank(),
   strip.text.y = element_blank(), text = element_text(size = 21), strip.text.x = element_text(size = 13), plot.margin = margin(10, 10, 10, 15)) 

ggsave('7d_left.png', p3)


p4 <- simulation_data_long_mt %>% 
  filter(type %in% c('New conjunctive cell input models', 'Experimental data')) %>%
  group_by(name) %>%
  filter(row_number() == 1) %>%
...
ggsave('7d_right.png', p4)


p <- ggbetweenstats(
  data = simulation_data_long_mt %>% filter(type %in% c('New conjunctive cell input models', 'Experimental data')),
  x = name,
...
write_csv(pb$plot$plot_env$df_pairwise, '7d_comparisons.csv')